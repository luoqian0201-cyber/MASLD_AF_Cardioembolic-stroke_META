...
The primary analysis used random-effects meta-analysis with restricted maximum-likelihood (REML) estimation to pool study-level hazard ratios, chosen to account for both within-study and between-study variability in observational data. The Hartung--Knapp adjustment was applied for the primary pooled estimate to yield more conservative confidence intervals. Heterogeneity was quantified using I\textsuperscript{2} and Cochran's Q, with I\textsuperscript{2} values of 25\%, 50\%, and 75\% interpreted as low, moderate, and high heterogeneity, respectively. The between-study variance component ($\tau^2$) was estimated using REML.
...
\textbf{Figure 7.} Fibrosis severity and dynamics. \textbf{(A)} Fibrosis-stratified stroke risk estimates across assessment tools ranging from histology and validated scores (NFS, BARD) to indirect biochemical surrogates (AAR). Shape coding: circle = low/reference, square = intermediate, triangle = advanced/high. Color coding by assessment tool. \textbf{(B)} Fibrosis dynamics (Park 2022): fibrosis regression (HR 0.58, protective) vs persistent fibrosis (HR 1.31).
