...
\textit{Meta-regression (Level I).} Meta-regression identified AF adjustment as a significant contributor to between-study heterogeneity (R\textsuperscript{2}=16.4\%, p=0.040; $\beta = -0.214$; Figure 6A). Studies adjusting for AF generally reported lower effect estimates than those without AF adjustment: the predicted stroke risk was HR 1.40 (95\% CI 1.31--1.49) without AF adjustment versus HR 1.13 (95\% CI 0.93--1.37) with AF adjustment, corresponding to 67.7\% attenuation of the excess risk.

\textit{Interaction by baseline AF (Level II).} In the only study reporting AF-stratified estimates, the association between MASLD and stroke remained significant among individuals without AF (HR 1.10, 95\% CI 1.08--1.11) but was attenuated among those with pre-existing AF (HR 1.03, 95\% CI 0.95--1.12; Figure 6B). Within-study AF covariate adjustment yielded consistent directional attenuation (HR 1.14 $\rightarrow$ 1.10; 28.6\% attenuation).
...
\textit{Additional evidence.} Lee et al. (2021) reported attenuation after bundle adjustment for cardiovascular comorbidities including AF (HR 1.55 $\rightarrow$ 1.43; 21.8\%), though the AF-specific contribution could not be isolated. Subgroup analyses yielded directionally consistent findings, although the number of studies in the AF-adjusted subgroup was limited (k=2).
...
Although subgroup differences according to fibrosis assessment did not reach conventional statistical significance (p=0.078), studies evaluating more advanced fibrosis phenotypes tended to report higher risk estimates (Supplementary Figure S9). Exploratory pooled estimates by fibrosis severity category are presented in Supplementary Material only, as fibrosis assessment tools were not harmonized across studies.
